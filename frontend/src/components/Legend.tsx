...
export function Legend() {
  ...
  const lut = isPrecip ? PRECIP_LUT_ALPHA : TEMP_LUT;
  const vmin = isPrecip ? PRECIP_MIN : TEMP_MIN;
  const vmax = isPrecip ? PRECIP_MAX : TEMP_MAX;
  const ticks = isPrecip ? PRECIP_TICKS : TEMP_TICKS;
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="flex justify-between text-[9px] text-slate-500 mt-3">
        {isPrecip ? (
          <>
            <span>{vmin} mm / 6h</span>
            <span>{vmax}+ mm / 6h</span>
          </>
        ) : (
          <>
            <span>{vmin}°C</span>
